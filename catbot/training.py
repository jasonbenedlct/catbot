...
def get_direct_move(bot_pos, cat_pos):
    bot_r, bot_c = bot_pos
    cat_r, cat_c = cat_pos
    
    # Close the vertical gap first
    if bot_r > cat_r: return 0  # Up
    if bot_r < cat_r: return 1  # Down
    
    # Close the horizontal gap
    if bot_c > cat_c: return 2  # Left
    if bot_c < cat_c: return 3  # Right
    
    # Fallback (should rarely hit this unless on top of the cat)
...
def train_bot(cat_name, render: int = -1):
    ...
    for ep in range(1, episodes + 1):
        ...
        while not done and steps < max_steps_per_episode:
            ...
            # Decode positions FIRST so we know where they are before moving
            bot_pos, cat_pos = decode_state(state)
            
            # 1. Smart Epsilon-Greedy Action Selection
            if random.random() < epsilon:
                if random.random() < 0.75:
                    # 75% of the time: Rush straight at the cat
                    action = get_direct_move(bot_pos, cat_pos)
                else:
                    # 25% of the time: Take a random step to break teleport loops
            ...
            if new_dist > prev_dist + 1:
                # Giving it a normal -1 step penalty so it isn't scared to push forward.
                reward = -1
